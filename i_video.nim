...
proc I_TranslateScancode(scancode: Scancode): cint {.exportc.} =
    case scancode:
        ...
        of SCANCODE_RALT:
            ...
            if sc >= 0 and sc < ScancodesSDLtoDoom.len:
                return ScancodesSDLtoDoom[scancode]
            else:
                return 0
...
proc I_ProcessEvent*(e: Event) {.exportc.} =
    ...
    case e.kind:
        of sdl.KeyDown:
            ...
            event.data1 = I_TranslateScancode(e.key.keysym.scancode)
            discard
        of sdl.KeyUp:
            event.`type` = ev_keyup
            event.data1 = I_TranslateScancode(e.key.keysym.scancode)
